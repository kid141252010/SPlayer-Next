...
export const parseTTML = (
  ...
  const rawSpatialOffsetMs = collectSpatialLyricOffset(doc);
  const spatialOffsetMs = options.applySpatialOffset ? rawSpatialOffsetMs : 0;
  if (options.onSpatialOffset && rawSpatialOffsetMs !== 0) {
    options.onSpatialOffset(rawSpatialOffsetMs);
  }
  ...
  if (spatialOffsetMs !== 0) {
    for (const line of lines) {
      line.startTime = Math.max(0, line.startTime + spatialOffsetMs);
      line.endTime = Math.max(0, line.endTime + spatialOffsetMs);
      for (const word of line.words) {
        word.startTime = Math.max(0, word.startTime + spatialOffsetMs);
        word.endTime = Math.max(0, word.endTime + spatialOffsetMs);
      }
    }
  }
